...
##postgress
db = SQLAlchemy()
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:root@localhost:3306/Details"
...
def borrow():
    if request.method == "POST":
        ...
        stud = Borrow(S_Name = name, B_title=BT, due=DA)
        db.session.add(stud)
        db.session.commit()
...
def rtrn():
    if request.method == "POST":
        ...
        stud = Rtrn(St_Name = name, Bo_title=BT, charges=DA)
        db.session.add(stud)
        db.session.commit()
